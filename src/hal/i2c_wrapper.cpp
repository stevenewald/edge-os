...
uint8_t i2c_reg_read(const nrf_twi_mngr_t* manager, uint8_t i2c_addr, uint8_t reg_addr)
{
    ...
    ret_code_t result = nrf_twi_mngr_perform(manager, NULL, read_transfer, 2, NULL);
...
void i2c_reg_write(const nrf_twi_mngr_t* manager, uint8_t i2c_addr, uint8_t reg_addr, uint8_t data)
{
    ...
    ret_code_t result =
        nrf_twi_mngr_perform(manager, NULL, write_transfer, 1, NULL);
...
void lsm303agr_init(const nrf_twi_mngr_t* manager)
{
    printf("Starting 303 init.\n");
    i2c_reg_write(manager, LSM303AGR_ACC_ADDRESS, CTRL_REG5_A, 0x80);
    nrf_delay_ms(100); // needs delay to wait for reboot

    printf("Checkpoint A.\n");
    // Enable Block Data Update
    // Only updates sensor data when both halves of the data has been read
    i2c_reg_write(manager, LSM303AGR_ACC_ADDRESS, CTRL_REG4_A, 0x80);

    // Configure accelerometer at 100Hz, normal mode (10-bit)
    // Enable x, y and z axes
    i2c_reg_write(manager, LSM303AGR_ACC_ADDRESS, CTRL_REG1_A, 0x57);

    // ---Initialize Magnetometer---

    // Reboot magnetometer
    i2c_reg_write(manager, LSM303AGR_MAG_ADDRESS, CFG_REG_A_M, 0x40);
    nrf_delay_ms(100); // needs delay to wait for reboot

    // Enable Block Data Update
    // Only updates sensor data when both halves of the data has been read
    i2c_reg_write(manager, LSM303AGR_MAG_ADDRESS, CFG_REG_C_M, 0x10);

    // Configure magnetometer at 100Hz, continuous mode
    i2c_reg_write(manager, LSM303AGR_MAG_ADDRESS, CFG_REG_A_M, 0x0C);

    i2c_reg_write(manager, LSM303AGR_ACC_ADDRESS, TEMP_CFG_REG_A, 0xC0);
}
